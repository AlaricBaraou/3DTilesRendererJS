...
const LAYER_COLORS = {
	// Nature & Water
	'water': 0x201f20, // Blue
	'waterway': 0x201f20, // Blue
	'landuse': 0xcaedc1, // Dark Green (Forests, Parks, etc.)
	'landuse_overlay': 0xcaedc1,
	'park': 0x5da859, // Lighter Green

	// Infrastructure
	'building': 0xeeeeee, // White/Grey buildings
	'road': 0x444444, // Dark roads
	'transportation': 0x444444,

	// Boundaries & Background
	'admin': 0x444545, // Red borders
	'background': 0x111111, // Dark background
	'default': 0x222222 // Fallback color
};
...
function inside( p, edge, val ) {
	...
	if ( edge === 2 ) return p.y >= val; // Top (0 in MVT is top usually, but let's stick to min/max)
...
function clipPolygonRing( subjectPolygon ) {
	...
	const edges = [
		{ edge: 0, val: 0 }, // Min X
		{ edge: 1, val: EXTENT }, // Max X
		{ edge: 2, val: 0 }, // Min Y
		{ edge: 3, val: EXTENT } // Max Y
	];
...
function clipLinePoints( p1, p2 ) {
	...
	if ( p1.x >= 0 && p1.x <= EXTENT && p1.y >= 0 && p1.y <= EXTENT &&
         p2.x >= 0 && p2.x <= EXTENT && p2.y >= 0 && p2.y <= EXTENT ) {

		return [ p1, p2 ];

	}

	// If complex clipping is needed, a dedicated line clipper is better,
	// but for MVT outlines, just dropping segments completely outside
	// or clamping usually suffices visually, or use a specific line clip lib.
	// For strict correctness, here is a simplified clip:

	let t0 = 0, t1 = 1;
	const dx = p2.x - p1.x;
	const dy = p2.y - p1.y;
	const p = [ - dx, dx, - dy, dy ];
	const q = [ p1.x, EXTENT - p1.x, p1.y, EXTENT - p1.y ];
...
export class MVTLoader extends MVTLoaderBase {
	...
	// Based on MVT spec: Y axis is positive DOWN.
	isExterior( ring ) {
