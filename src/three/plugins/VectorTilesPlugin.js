// src\three\plugins\VectorTilesPlugin.js
...
export class VectorTilesPlugin extends XYZTilesPlugin {
	...
	constructor( options = {} ) {
		...
		this.globeMesh.renderOrder = - 9999; // Render first
	...
	async parseToMesh( buffer, tile, extension, uri, abortSignal ) {
		...
		if ( extension === 'pbf' || extension === 'mvt' ) {
			...
			// Project the loaded geometry onto the globe
	...
	_projectGroupToGlobe( group, tile ) {
		...
		const extents = 4096; // Standard MVT extent

		// 1. Calculate the Tile Center (RTC Origin)
		// We place the Group at the cartesian center of the tile.
		// All vertex positions will be relative to this point to prevent floating point jitter.
		...
		group.traverse( child => {
			...
			if ( child.isMesh || child.isLineSegments || child.isPoints ) {
				...
				// Important: bounding boxes must be updated for frustum culling to work
				// on the now-curved geometry
