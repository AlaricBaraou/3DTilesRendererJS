...
const LAYER_COLORS = {
	...
	'admin': 0x444545,
	'background': 0x111111,
	// ADDED: distinct color for POIs so they are visible
	'poi': 0xFFCC00,
	'default': 0x222222
};
...
export class MVTImageSource extends XYZImageSource {
	...
	constructor( options = {} ) {
		...
		console.log( '[MVTImageSource] Initializing MVT Image Source with options:', options );
		...
		this._styles = {};
		for ( const key in LAYER_COLORS ) {

			_color.setHex( LAYER_COLORS[ key ] );
	...
	/**
     * Helper to create an OffscreenCanvas if available,
     * otherwise fall back to a standard DOM Canvas.
     */
					...
					for ( const multiPoint of geometry ) {
						...
						for ( const p of multiPoint ) {
							...
							if ( isLabelLayer ) {
								...
								// You might want to skip labels if they are too close to edges
								// or if you want to implement a collision grid later.
								// if ( labelText ) {
								...
								// (This is your existing logic for non-text points)
								const radius = ( layerName === 'poi' ) ? 3 : 2;

								ctx.beginPath();
								ctx.moveTo( x + radius, y );
								ctx.arc( x, y, radius, 0, Math.PI * 2 );
								ctx.fillStyle = color; // Reset color to layer color
								ctx.fill();
